"""
STREAMING UPIT 5: Pritisak vazduha u Pekingu

Ovaj streaming job obradjuje podatke iz Kafka stream-a i prati
kako se trenutni pritisak u poslednjih 60 minuta poredi sa
istorijskim prosekom za isti mesec (jun) u periodu 1990–1999.

Na osnovu toga se racunaju apsolutna i procentualna odstupanja.
"""
...
def create_es_index():
    url = "http://elasticsearch:9200/realtime_beijing_pressure"
...
PG_TABLE     = "realtime_beijing_pressure"
TARGET_CITY  = "Beijing"

# Ucitavanje istorijskih podataka (1990–1999) za referentni mesec
# Ovi podaci se koriste kao osnova za poredjenje sa real-time stream-om
...
hist_ref = (
    ...
    .groupBy("year", "month")
    .agg(round(avg("pressure_msl"), 3).alias("hist_avg_pressure"))
    .orderBy("year")
)
...
# Ucitavanje real-time podataka iz Kafka stream-a
# Svaka poruka predstavlja trenutne meteoroloske uslove po gradu
...
# Agregacija streaming podataka u 60-minutne klizne prozore
# sa update-om na svakih 5 minuta i tolerancijom na kasnjenje podataka od 5 minuta
windowed = (
    weather
...
# - agregirani streaming podaci iz window-a
# - spajanje sa istorijskim referencama (1990–1999)
# - racunanje odstupanja (apsolutno i procentualno)
# - upis rezultata u Elasticsearch po batch-u
...
# Spajanje streaming agregiranih podataka sa istorijskim podacima po godinama
# kako bi se dobilo poredjenje trenutnog i istorijskog proseka
    result = (
        result_base
        .crossJoin(hist_spark)
        .withColumn("abs_deviation",
            round(col("stream_avg_pressure") - col("hist_avg_pressure"), 3))
        .withColumn("pct_deviation",
            round(
                (col("stream_avg_pressure") - col("hist_avg_pressure"))
                / col("hist_avg_pressure") * 100.0,
            3))
        ...
        .select(
            ...
            "stream_avg_pressure", "hist_avg_pressure",
    ...
    print(f"\n{'='*70}")
    print(f"[Job5] Batch {batch_id}  |  {TARGET_CITY} — pritisak vs istorijski prosek (60-min window)")
    print(f"{'='*70}")
    for row in rows:
    ...
    result.write \
        ...
        .option("es.port", "9200") \
        .option("es.resource", "realtime_beijing_pressure") \
...
query = windowed.writeStream \
    ...
    .option("checkpointLocation", "hdfs://namenode:9000/checkpoints/q5_beijing_pressure") \
    .start()
